...
namespace ZFreeGo.ChoicePhase.ControlCenter
{
    public partial class MainWindow
    {
        ...
        private void YongciOpenSerialPort_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                if (!yongciSerialControlCenter.CommState)
                {
                    yongciSerialControlCenter.SerialPort.PortName = portNameB.SelectedItem as string;
                    yongciSerialControlCenter.SerialPort.BaudRate = (int)baudRateB.SelectedItem;
                    yongciSerialControlCenter.SerialPort.DataBits = (int)dataBitsB.SelectedItem;
                    yongciSerialControlCenter.SerialPort.Parity = (Parity)portParityB.SelectedItem;

                    yongciSerialControlCenter.SerialPort.StopBits = (StopBits)Enum.Parse(typeof(StopBits), stopBits.SelectedItem.ToString());

                    yongciSerialControlCenter.OpenSerialPort();
                    openSerialPortB.Content = "关闭串口";

                    commBGrid.IsEnabled = false;

                    systemState.CommonStateB = "串口正常";
                ...
                else
                {
                    ...
                    yongciSerialControlCenter.CloseSerialPort();
                    openSerialPortB.Content = "打开串口";
                    commBGrid.IsEnabled = true;
                    systemState.CommonStateB = "串口关闭";
        ...
        public void UpdatePortShowB(SerialPort serialPort)
        {
            //永磁控制通讯
            UpdatePortName(portNameB, serialPort.PortName);
            UpdatePortBaudRate(baudRateB, serialPort.BaudRate);
            UpdatePortDataBits(dataBitsB, serialPort.DataBits);
            UpdatePortParity(portParityB, serialPort.Parity);
            UpdatePortStopBits(stopBitsB, serialPort.StopBits);
        }
